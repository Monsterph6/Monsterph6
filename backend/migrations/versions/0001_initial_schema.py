...
# Each enum below backs exactly one column in exactly one table, so we let
# op.create_table/op.drop_table create and drop the PostgreSQL type automatically
# (the default behavior) instead of also creating it explicitly — doing both raises
# "type already exists" on PostgreSQL (SQLite has no native enum type, so this bug
# only surfaces when running against real Postgres, not the SQLite test suite).
...
def downgrade() -> None:
    # Dropping each table also drops its column's enum type automatically (the
    # default create_type=True behavior covers DROP TYPE symmetrically).
